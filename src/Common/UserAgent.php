...
class userAgent {
     ...
     */
    public array $windows_os = [ '[Windows; |Windows; U; |]Windows NT 6.:number0-3:;[ Win64; x64| WOW64| x64|]',
                           '[Windows; |Windows; U; |]Windows NT 10.:number0-5:;[ Win64; x64| WOW64| x64|]', ];
    /**
     * Linux Operating Systems [limited]
     * @var array $linux_os
     */
    public array $linux_os = [ '[Linux; |][U; |]Linux x86_64',
     ...
     */
    public array $androidDevices = [ '4.3' => [ 'GT-I9:number2-5:00 Build/JDQ39',
    ...
    /**
     * Get Mobile OS
     * @param null $os Can specify android, iphone, ipad, ipod, or null/blank for random
     * @return array|string *
     * @throws \Random\RandomException
     */
    public function getMobileOS($os = NULL): array|string
    {
        $os = strtolower($os);
        $_os = [];
        switch( $os ) {
    ...
    public function generate($userAgent = NULL): string
    {
        ...
        elseif($userAgent == 'firefox') {
            return 'Mozilla/5.0 (' . $this->getOS($userAgent) . ') Gecko/' . (random_int(1, 100) > 30 ? '20100101' : '20130401') . ' Firefox/' . self::firefoxVersion([ 'min' => 45,'max' => 74 ]);
        }
